...
def load_kurtz_g():
    ...
    with open('splittings.kurtz') as f:
        for line in f:
            arr = line.split()
            if len(arr)==0: break
            n = 100*int(arr[0]) + int(arr[1])
            if not n in freq: freq[n] = []
            freq[n].append( (1.e9*cd_to_Hz(float(arr[3])), 1.e9*cd_to_Hz(float(arr[5]))) )
    ...
    with open('splittings.kurtz') as f:
        for line in f:
            ...
            if l != 6:
                ...
                freq[nl].append( (1.e9*cd_to_Hz(float(arr[3])), 1.e9*cd_to_Hz(float(arr[5]))) )
...
if __name__=='__main__':
    ...
    J = load_Jordan_split(left=False)
    for key in J:
        print(key, J[key])
